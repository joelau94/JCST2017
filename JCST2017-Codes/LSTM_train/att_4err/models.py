...
class Classifier(object):
	...
	def __call__(self, decoder_input, hidden_tape):
		"""
		decoder_input: shape (1,word_emb_dim+2*hidden_dim+max_seq_len) [input;hidden_state;mask]
		hidden_tape: shape (sent_len, 2*hidden_dim)
		"""
		...
		mask = tf.slice(decoder_input,[0,word_emb_dim+2*hidden_dim],[1,max_seq_len])

		tape_pad = tf.zeros([max_seq_len,2*hidden_dim])

		hidden_tape_padded = tf.slice(tf.concat(0,[hidden_tape,tape_pad]),[0,0],[max_seq_len,2*hidden_dim])
		# shape: (sent_len,2*hidden_dim)(max_seq_len,2*hidden_dim) --concat-->
		#        (sent_len+max_seq_len, 2*hidden_dim) --slice-->
		#        (max_seq_len,2*hidden_dim)
		
		# attention

		s = tf.matmul(hidden_tape_padded,hidden_state,transpose_b=True)
		# shape: (max_seq_len,1) = (max_seq_len, 2*hidden_dim) dot (2*hidden_dim, 1)
		
		s = tf.reshape(s,[1,-1])
		# shape (1,max_seq_len) : legacy: that's how tf.nn.softmax could work
		attn_weights = masked_softmax(s,mask)
		# shape (1,max_seq_len)

		attn_context = tf.reduce_sum(tf.reshape(attn_weights,[-1,1]) * hidden_tape_padded, 0, keep_dims=True)
		# shape (max_seq_len,1) * (max_seq_len,2*hidden_dim) --> (max_seq_len,2*hidden_dim)
		#       --reduce_sum--> (1,2*hidden_dim)
		...
		logit_and_attn = tf.concat(1,[logit,attn_weights])
...
class GEDModel(object):
	def __init__(self, is_training, config):
		...
		# shape [unknown(sentence length),]
		self.mask = tf.placeholder(tf.float32, shape=[None,max_seq_len], name="mask")
		# shape [unknown(sentence length),unknown(sentence length)]
		...
		decoder_input = tf.concat(1,[inputs,self.hidden_tape,self.mask])
		...
		with tf.variable_scope("classifier") as scope:
			...
			def decode_step(logit_and_attn,x):
				x = tf.reshape(x,[1,word_emb_dim+2*hidden_dim+max_seq_len])
