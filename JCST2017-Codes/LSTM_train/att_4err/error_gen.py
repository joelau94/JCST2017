...
err_type_pos = ["DT", "PDT", "IN", "TO", "RP", "NN", "NNP", "NNPS", "NNS", "VB", "VBD", "VBG", "VBN", "VBP", "VBZ"]
...
with open(text_file, "r") as corr:
	...
	for line in corr:
		...
		if len(tok):
			...
			repl_flag = False
			repl_time = 0
			while repl_flag == False and repl_time < 10:
				repl_time += 1
				error_pos = random.randrange(0,len(tok),1)
				pos = nltk.pos_tag([tok[error_pos]])[0][1]
				if pos in err_type_pos:
					repl_flag = True
