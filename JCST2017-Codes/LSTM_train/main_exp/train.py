...
def train(sess, epoch, model):
	...
	for i in shuffled_order:
		sent_len = len(indices[i])
		mask = []
		for j in range(sent_len):
			mask.append([1]*sent_len + [0]*(max_seq_len-sent_len))
			mask[j][j] = 0

		mask = [map(float,x) for x in mask]
...
with tf.Graph().as_default(), tf.Session() as sess:
	...
	while (costs == 0.0 or math.fabs(costs - new_costs) >= rel_tol * costs) and iteration < max_it:
		...
		for i in range(1, datasets+1):
			...
			print("Iter: %i, Dataset: %i, Training Cost: %.3f (%s)" % (iteration, i, new_costs, str(datetime.datetime.now())))
			sys.stdout.flush()

			# test after each epoch
			log.write("Testing: Iter={}, Dataset={} ({})\n".format(iteration,i,str(datetime.datetime.now())))
			log.flush()
			# os.fsync
