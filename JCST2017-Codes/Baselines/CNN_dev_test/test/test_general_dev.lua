...
function options()
	...
	cmd:option('-dataFile', 'data.csv', 'File name of data file')
	cmd:option('-labelFile', 'lbl.csv', 'File name of label file')
...
function test()
	...
	-- dataGenCmd = "python src2vec_general.py "..opt.vectorSize.." "..opt.windowSize.." "..opt.dict.." "..opt.langMdl.." "..opt.srcFile.." ".."./".." "..opt.logFile
	-- call src2vec.py to generate input vector file
	os.execute(dataGenCmd)
	-- load dataset
	local dataset = load_data(opt.dataFile, opt.labelFile, opt.windowSize)	
	print('Loading model ... ')
	-- load model
